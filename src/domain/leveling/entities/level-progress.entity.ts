...
export class LevelProgress {
  ...
  get totalXp():      number { return this._totalXp.value;      }
  get currentLevel(): number { return this._currentLevel.value; }
  get xpMultiplier(): number { return this.props.xpMultiplier;  }
  get tier():         Tier   { return this._currentLevel.getTier();       }
  get tierNumber():   number { return this._currentLevel.getTierNumber(); }
  get tierName():     string { return this._currentLevel.getTierName();   }

  getXpProgress(): { currentLevelXp: number; nextLevelXp: number; progress: number } {
  ...
  addExperience(rawAmount: number): Result<XpGainResult, InvalidXpAmountError> {
    ...
    const newLevelValue   = XpCalculationDomainService.calculateLevelFromXp(newXp.value);
