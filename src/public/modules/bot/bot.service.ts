...
export class BotService {
  ...
  async getWithdrawingItems(username: string, ownerBotId: number) {
    ...
    const items = await this.prisma.userInventoryAmp.updateManyAndReturn({
      where: {
  ...
  async processWithdraw(data: SucesfullWithdrawDTO) {
    ...
    try {
      const result = await this.prisma.userInventoryAmp.findMany({
        where: {
          userUsername: user.username,
          owner_bot_id: data.ownerBotId,
          petInGameId: {
      ...
      const { count } = await this.prisma.userInventoryAmp.deleteMany({
        where: {
          id: {
    ...
    this.redis.del(RedisKeys.user.balance.value(data.username));
      this.logger.log(
        JSON.stringify({
          ...
          username: data.username,
          ownerBotId: data.ownerBotId,
          requestedItems: data.itemIds.length,
          deletedItemsCount: count,
          itemNames: result.map((item) => item.pet.name),
        }),
      );
  ...
  async processWithdrawDecline(data: WithdrawDeclineDTO) {
    ...
    try {
      const result = await this.prisma.userInventoryAmp.updateMany({
        where: {
          userUsername: {
            equals: data.username,
            mode: 'insensitive',
