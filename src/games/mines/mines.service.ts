...
export class MinesService {
  ...
  async revealTile(username: string, gameId: string, tile: number) {
    ...
    this.updateGameHistory(gameId, game, {
      ...
      profit: !active ? (hitMine ? -game.betAmount : (game.betAmount * multiplier) - game.betAmount) : undefined,
    ...
    if (!active && outcome) {
      this.updateGameStatistics(username, game.betAmount, multiplier, outcome).catch(err => {
  ...
  async cashout(username: string, gameId: string) {
    ...
    this.updateGameHistory(gameId, game, {
      ...
      this.logger.error(`Failed to update game history for cashout ${gameId}:`, err);
    });

    // Update statistics
    this.updateGameStatistics(username, game.betAmount, game.multiplier, 'CASHED_OUT').catch(err => {
      this.logger.error(`Failed to update statistics:`, err);
    });
  ...
  /**
   * Get comprehensive seed dashboard (delegates to SeedManagementService)
   */
  async getSeedDashboard(username: string) {
    return await this.seedManagement.getSeedDashboard(username);
  }
    ...
    try {
      await this.prisma.gameHistory.create({
        data: {
          ...
          seedRotationHistoryId: null, // Not rotated yet
          ...
          // Financial data
          betAmount: new Decimal(gameData.betAmount),
          finalMultiplier: new Decimal(gameData.multiplier),
          payout: new Decimal(0),
          profit: new Decimal(-gameData.betAmount),
  ...
  private async updateGameHistory(
    ...
    try {
      ...
      const currentGame = await this.prisma.gameHistory.findUnique({
        where: { gameId },
        select: { gameData: true, startedAt: true },
      });
      ...
      // Prepare update data
      const updateData: Prisma.GameHistoryUpdateInput = {};

      // Update gameData JSON field
      updateData.gameData = {
        revealedTiles: updates.revealedTiles || currentGameData.revealedTiles,
        minePositions: currentGameData.minePositions, // Keep mines hidden until game ends
        cashoutTile: updates.cashoutTile !== undefined ? updates.cashoutTile : currentGameData.cashoutTile,
      } as Prisma.JsonObject;

      // If game ended, reveal mine positions
      if (updates.completedAt) {
        (updateData.gameData as any).minePositions = this.maskToTileArray(gameData.mineMask);
      }

      // Update multiplier
      if (updates.multiplier !== undefined) {
        updateData.finalMultiplier = new Decimal(updates.multiplier);
      }
      ...
      if (updates.completedAt) {
        ...
        updateData.duration = Math.floor(
          (updates.completedAt.getTime() - currentGame.startedAt.getTime()) / 1000
        );
      }

      // Update financial data
      if (updates.payout !== undefined) {
        updateData.payout = new Decimal(updates.payout);
      }

      if (updates.profit !== undefined) {
        updateData.profit = new Decimal(updates.profit);
      }
    ...
    try {
      ...
      await this.prisma.userGameStatistics.upsert({
        ...
        create: {
          ...
          gameType: 'MINES',
          totalGames: 1,
          totalWagered: new Decimal(betAmount),
          totalPayout: new Decimal(payout),
          totalProfit: new Decimal(profit),
          gamesWon: outcome === 'WON' ? 1 : 0,
          gamesLost: outcome === 'LOST' ? 1 : 0,
          gamesCashedOut: outcome === 'CASHED_OUT' ? 1 : 0,
          biggestWin: outcome !== 'LOST' ? new Decimal(profit) : new Decimal(0),
          biggestLoss: outcome === 'LOST' ? new Decimal(betAmount) : new Decimal(0),
          highestMultiplier: new Decimal(multiplier),
        ...
        update: {
          ...
          gamesCashedOut: outcome === 'CASHED_OUT' ? { increment: 1 } : undefined,
          lastGameAt: new Date(),
        },
      });
