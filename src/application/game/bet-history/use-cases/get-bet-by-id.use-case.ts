...
@Injectable()
export class GetBetByIdUseCase implements IUseCase<
  GetBetByIdInput,
  Result<BetHistoryItemOutputDto, BetHistoryError>
> {
  ...
  async execute(
    ...
    try {
      const record = await this.betHistoryRepo.findByIdAndUsername(
        gameId,
        username,
      );
