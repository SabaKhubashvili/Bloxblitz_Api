...
@Injectable()
export class GetDailySpinStatusUseCase implements IUseCase<
  GetDailySpinStatusQuery,
  Result<DailySpinStatusOutputDto, DailySpinError>
> {
  ...
  async execute(
    ...
    const state =
      (await this.repo.findStateByUsername(query.username)) ??
    ...
    void this.cache
      ...
      .catch((err) =>
        this.logger.warn(
          `[DailySpin] Status cache write failed for ${query.username}`,
          err,
        ),
      );
