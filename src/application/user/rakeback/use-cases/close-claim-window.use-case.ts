...
@Injectable()
export class CloseClaimWindowUseCase implements IUseCase<
  CloseWindowInput,
...
> {
  ...
  async execute(
    ...
    this.logger.log(
      `Closing ${input.type} claim window — resetting missed streaks`,
    );

    const affected = await this.repo.resetMissedStreaks(input.type);
    this.logger.log(
      `${input.type} window closed — ${affected} streak(s) reset`,
    );
