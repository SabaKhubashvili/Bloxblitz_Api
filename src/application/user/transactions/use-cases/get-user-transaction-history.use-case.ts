 ...
 */
@Injectable()
export class GetUserTransactionHistoryUseCase implements IUseCase<
  GetTransactionHistoryQuery,
  Result<TransactionHistoryOutputDto, TransactionError>
> {
  ...
  async execute(
    ...
    const filters: TransactionHistoryFilters = {
      ...
      direction:
        direction && VALID_DIRECTIONS.has(direction) ? direction : undefined,
      status: status && VALID_STATUSES.has(status) ? status : undefined,
      assetType:
        assetType && VALID_ASSET_TYPES.has(assetType) ? assetType : undefined,
      from: fromDate,
      to: toDate,
    };
    ...
    try {
      ...
      return Ok({
        ...
        limit,
        totalPages:
          pageData.total === 0 ? 0 : Math.ceil(pageData.total / limit),
      });
    } catch (err) {
      this.logger.error(
